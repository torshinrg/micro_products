...
export default function SplitTrainingApp() {
  ...
  const startTimer = (index, time) => {
    setTimers((prevTimers) => ({ ...prevTimers, [index]: time }));
    const interval = setInterval(() => {
      setTimers((prevTimers) => {
        if (prevTimers[index] === undefined || prevTimers[index] <= 0) {
          clearInterval(interval);
          return { ...prevTimers, [index]: 0 };
        }
        return { ...prevTimers, [index]: prevTimers[index] - 1 };
      });
    }, 1000);
  };
          ...
          <Card key={index} className="mb-2 p-2 bg-gray-800 text-white">
            <CardContent className="flex justify-between items-center text-left">
              ...
              <p 
                ...
                onClick={() => startTimer(index, exercise.time)}
              >{timers[index] !== undefined ? timers[index] : exercise.time}s</p>
            </CardContent>
          </Card>
        ))}
